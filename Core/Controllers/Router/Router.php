	...
	public function findMatch($requestUrl) {
		...
		foreach ($allRoutes as $route) {
			if(count($route->getVerbs()) == 0 || in_array($this->request->getMethod(), $route->getVerbs())) {
				...
				$matchFound = true;
				GFEventController::dispatch(ROUTER_MATCH_SUCCESS);
				$this->request->setHasMatch(true);
				$this->request->setMatchedRoute($route);
				GFEventController::dispatch(ROUTER_PARSE_PARAMS_BEFORE);
				$this->request->parseIncomingParams();
				GFEventController::dispatch(ROUTER_PARSE_PARAMS_AFTER);
		...
		if($matchFound == false) {
			...
			GFEventController::dispatch(ROUTER_MATCH_FAILED);
		}
	}
